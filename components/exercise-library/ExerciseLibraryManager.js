...
const ExerciseLibraryManager = () => {
  const [exercises, setExercises] = useState(exerciseLibrary.getAllExercises())
  ...
  const categories = ["All", ...exerciseLibrary.getAllCategories()]
  ...
  const handleDeleteExercise = (id) => {
    if (confirm("Are you sure you want to remove this exercise from the library?")) {
      exerciseLibrary.deactivateExercise(id)
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
        ...
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
          {categories.map((cat) => (
            <Chip
              ...
              sx={{
                backgroundColor: filterCategory === cat ? "#2E5E99" : "transparent",
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={exercise.id}>
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  <Typography variant="h6" sx={{ fontWeight: 600, mb: 1, color: "#2E5E99" }}>
                    {exercise.name}
                  </Typography>
                  ...
                  <Box sx={{ mb: 2 }}>
                    <Chip
                      ...
                      size="small"
                      sx={{ backgroundColor: "#e8f5e9", color: "#2E5E99", mr: 1 }}
                    />
                    <Chip
                      label={exercise.difficulty}
                      size="small"
                      sx={{
                        backgroundColor: exercise.difficulty === "beginner" ? "#c8e6c9" : "#fff9c4",
                        color: "#2E5E99",
                      }}
                    />
                  </Box>
                  ...
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mb: 2 }}>
                    {exercise.targetMuscles.slice(0, 2).map((muscle, idx) => (
                      <Typography key={idx} variant="caption" sx={{ color: "#2E5E99", fontWeight: 500 }}>
                        {muscle}
      ...
      {/* Add Exercise Dialog */}
      <AddExerciseDialog open={openAddDialog} onClose={handleCloseAddDialog} onExerciseAdded={handleAddExercise} />
          ...
          <DialogContent sx={{ pt: 3 }}>
            {selectedExercise.imageUrl && (
              <CardMedia
                ...
                sx={{ mb: 2, borderRadius: 1, maxHeight: "300px", objectFit: "cover" }}
              />
            )}
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {selectedExercise.targetMuscles.map((muscle, idx) => (
                  <Chip key={idx} label={muscle} size="small" sx={{ backgroundColor: "#e8f5e9", color: "#2E5E99" }} />
                ))}
              </Box>
            </Box>
